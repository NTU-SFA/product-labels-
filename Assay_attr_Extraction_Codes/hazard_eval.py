...
# ===== 待抽取的原始 batch 文件 ====（可用 INPUT_FILE 环境变量覆盖）
INPUT_JSON_PATH = os.environ.get(
    "INPUT_FILE",
    os.path.join(RASFF_ROOT, "FIND-food-recall-data-main_V2", "rasff_data_2024_batch1.json"),
)
...
OUTPUT_DIR = os.path.join(CODE_DIR, "Outputs_hazard_predict_rule_only_v6")
PREDICTION_OUTPUT_PATH = os.path.join(OUTPUT_DIR, "rasff_batch_with_hazard_predictions_v6.json")
ERROR_OUTPUT_PATH = os.path.join(OUTPUT_DIR, "rasff_batch_hazard_prediction_errors_v6.json")
SUMMARY_OUTPUT_PATH = os.path.join(OUTPUT_DIR, "rasff_batch_hazard_prediction_summary_v6.json")

# 调试时可设为 20 / 100；正式跑全量设为 None
DEBUG_N = None
...
def main():
    ...
    for idx, item in enumerate(tqdm(batch_data, desc="Predicting hazard labels for raw batch")):
        time.sleep(0.003)
        source_stats["total_records"] += 1
        ...
        try:
            ...
            if has_hazards:
                ...
                for h in hazards:
                    ...
                    labels, cats, local_source = map_has_hazard_item(
                        raw_hazard=raw_hazard,
                        raw_category=raw_category,
                ...
                if "exact_raw_hazard" in per_item_sources:
                    ...
                    prediction_source = "has_hazards_canonical_fallback"
            ...
            if idx < 5:
                ...
                print(json.dumps({
                    ...
                    "matched_mapping_keys": sorted(list(set(matched_keys)))
    ...
    print(f"Errors saved to: {ERROR_OUTPUT_PATH}")
